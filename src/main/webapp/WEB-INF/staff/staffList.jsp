          	...
          	<form id="searchStaffForm" name="searchStaffForm" action="${ctx}/staff/staffList" method="post">
                    <ul>
                      ...
                      <li ><input type="text" name="username" id="username"
                                  class="cha1"  value="${username}"/></li>
                      <li ><input class="button" type="submit" name="button" id="button" value="查找"  style="cursor:pointer;"/></li>
                      <li ><a class="button2" id="button" href="${ctx}/staff/toAddStaff">新增用户</a></li>
                    </ul>
                    </form>
                </div><!--输入用户名-->
                      ...
                      <table width="1000" border="0"  class="dsfd yuan" >
                          <c:forEach var="staff" items="${staffList}" varStatus="status">
                              <tr>
                                ...
                                <td width="100">${staff.email}</td>
                                <td width="80">${staff.leader}</td>
                                <td width="80"><c:if
                                        test="${staff.manageLevel==4}">普通员工
                                </c:if><c:if test="${staff.manageLevel==3}">管理员</c:if></td>
                                <td width="60"><c:if
                                        test="${staff.userStatus==0}">在职
                                </c:if><c:if test="${staff.userStatus==1}"><span style="color:red">离职</span></c:if></td>
                                 <td width="100"><fmt:formatDate
                                         value="${staff.createTime}" pattern="yyyy-MM-dd"/> </td>
                                <td width="80"
                                    style="cursor:pointer;"><a
                                        href="${ctx}/staff/toEditStaff?id=${staff.id}">编辑</a></td>
                              </tr>
                                 </c:forEach>
                      </table>
               ...
               <ul class="page">
					    ...
					    <a href="${ctx}/staff/staffList?pageNo=${pageNo}" style="background: #C30D23 none repeat scroll 0% 0%;color:#ffffff;">${pageNo}</a>
					     </c:when>
					   <c:otherwise>
					   <a href="${ctx}/staff/staffList?pageNo=${pageNo}" >${pageNo}</a>
					   </c:otherwise>
					   </c:choose>
